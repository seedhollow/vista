		...
		if (mode_ == Mode::Il2Cpp) {
			...
			do {
				if ((method = Invoke<void*>("il2cpp_class_get_methods", pKlass, &iter))) {
					...
					pMethod->function = method ? *static_cast<void**>(method) : nullptr;
...
public:
	...
	public:
		...
		struct Quaternion {
			...
			static auto EulerAngles(float m_fX, float m_fY, float m_fZ) -> Quaternion { return Quaternion().Euler(m_fX, m_fY, m_fZ); }
		...
		struct Shader : Object {
			static auto Find(const std::string& name) -> Shader* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Shader")->Get<Method>("Find", { "System.String" });
			...
			static auto PropertyToID(const std::string& name) -> int {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Shader")->Get<Method>("PropertyToID", { "System.String" });
		...
		struct Material : Object {
			auto GetShader() -> Shader* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("get_shader");
			...
			auto SetShader(Shader* shader) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("set_shader", { "UnityEngine.Shader" });
				if (method) return method->Invoke<void>(this, shader);
			}

			auto HasProperty(const std::string& name) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("HasProperty", { "System.String" });
				if (method) return method->Invoke<bool>(this, String::New(name));
				return false;
			}

			auto HasProperty(const int nameID) -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("HasProperty", { "System.Int32" });
				if (method) return method->Invoke<bool>(this, nameID);
				return false;
			}

			auto SetColor(const std::string& name, const Color& value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetColor", { "System.String", "UnityEngine.Color" });
				if (method) return method->Invoke<void>(this, String::New(name), value);
			}

			auto SetColor(const int nameID, const Color& value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetColor", { "System.Int32", "UnityEngine.Color" });
				if (method) return method->Invoke<void>(this, nameID, value);
			}

			auto SetFloat(const std::string& name, const float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetFloat", { "System.String", "System.Single" });
				if (method) return method->Invoke<void>(this, String::New(name), value);
			}

			auto SetFloat(const int nameID, const float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetFloat", { "System.Int32", "System.Single" });
				if (method) return method->Invoke<void>(this, nameID, value);
			}

			auto GetFloat(const std::string& name) -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("GetFloat", { "System.String" });
				if (method) return method->Invoke<float>(this, String::New(name));
				return {};
			}

			auto GetFloat(const int nameID) -> float {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("GetFloat", { "System.Int32" });
				if (method) return method->Invoke<float>(this, nameID);
				return {};
			}

			auto SetTexture(const std::string& name, Object* value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetTexture", { "System.String", "UnityEngine.Texture" });
				if (method) return method->Invoke<void>(this, String::New(name), value);
			}

			auto SetTexture(const int nameID, Object* value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("SetTexture", { "System.Int32", "UnityEngine.Texture" });
				if (method) return method->Invoke<void>(this, nameID, value);
			}

			auto GetColor(const std::string& name) -> Color {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("GetColor", { "System.String" });
				if (method) return method->Invoke<Color>(this, String::New(name));
				return Color();
			}

			auto GetColor(const int nameID) -> Color {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Material")->Get<Method>("GetColor", { "System.Int32" });
				if (method) return method->Invoke<Color>(this, nameID);
				return Color();
			}
		};
		...
		struct Application : Object {
			static auto SetTargetFrameRate(const int frameRate) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Application")->Get<Method>("set_targetFrameRate", { "System.Int32" });
				if (method) return method->Invoke<void>(frameRate);
				return;
			}

			static auto GetTargetFrameRate() -> int {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Application")->Get<Method>("get_targetFrameRate");
				if (method) return method->Invoke<int>();
				return 0;
			}

			static auto GetVersion() -> String* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Application")->Get<Method>("get_version");
		...
		template <typename T>
		struct SettingParameter : Object {
			static T paramValue_k__BackingField;
			static T defaultValue_k__BackingField;

			// Methods
			void ctor(T defaultVal, UnityType::String* featureName, UnityType::String* paramName) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>(".ctor", { "*", "System.String", "System.String" });
				if (method) method->Invoke<void>(this, defaultVal, featureName, paramName);
			}

			static SettingParameter<T>* Create(T defaultVal, UnityType::String* featureName, UnityType::String* paramName) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("Create", { "*", "System.String", "System.String" });
				if (method) return method->Invoke<SettingParameter<T>*>(defaultVal, featureName, paramName);
				return nullptr;
			}

			T get_paramValue() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("get_paramValue");
				if (method) return method->Invoke<T>(this);
				return T();
			}

			void set_paramValue(T value) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("set_paramValue", { "*" });
				if (method) method->Invoke<void>(this, value);
			}

			T get_defaultValue() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("get_defaultValue");
				if (method) return method->Invoke<T>(this);
				return T();
			}
			...
			void set_defaultValue(T value) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("set_defaultValue", { "*" });
				if (method) method->Invoke<void>(this, value);
			}

			void Override(T val) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("Override", { "*" });
				if (method) method->Invoke<void>(this, val);
			}

			bool IsOverrided() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("IsOverrided");
				if (method) return method->Invoke<bool>(this);
				return false;
			}
			...
			bool IsFromCode() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("IsFromCode");
				if (method) return method->Invoke<bool>(this);
				return false;
			}

			bool ChangeParamValue(UnityType::String* valueString, bool markOverrided) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("ChangeParamValue", { "System.String", "System.Boolean" });
				if (method) return method->Invoke<bool>(this, valueString, markOverrided);
				return false;
			}

			void RefreshInternal() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("RefreshInternal");
				if (method) method->Invoke<void>(this);
			}

			bool OverrideWithString(UnityType::String* valueString) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("OverrideWithString", { "System.String" });
				if (method) return method->Invoke<bool>(this, valueString);
				return false;
			}

			static T op_Implicit(SettingParameter<T>* p) {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("op_Implicit", { "HG.Rendering.Runtime.SettingParameter`1" });
				if (method) return method->Invoke<T>(p);
				return T();
			}

			UnityType::String* ToString() {
				static Method* method;
				if (!method) method = Get("HG.RenderPipelines.Runtime.dll")->Get("SettingParameter`1", "HG.Rendering.Runtime")->Get<Method>("ToString");
				if (method) return method->Invoke<UnityType::String*>(this);
				return nullptr;
			}
		};
		...
		struct Rigidbody : Component {
			...
			auto SetCollisionDetectionMode(const int value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("set_collisionDetectionMode", { "*" });
				if (method) return method->Invoke<void>(this, value);
			}

			// gravity
			auto GetUseGravity() -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("get_useGravity");
				if (method) return method->Invoke<bool>(this);
				return false;
			}

			auto SetUseGravity(const bool value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("set_useGravity");
				if (method) return method->Invoke<void>(this, value);
			}

			// isKinematic
			auto GetIsKinematic() -> bool {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("get_isKinematic");
				if (method) return method->Invoke<bool>(this);
				return false;
			}

			auto SetIsKinematic(const bool value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.PhysicsModule.dll")->Get("Rigidbody")->Get<Method>("set_isKinematic");
				if (method) return method->Invoke<void>(this, value);
			}
		};
		...
		struct Renderer : Component {
			...
			auto GetMaterial() -> Material* {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Renderer")->Get<Method>("get_material");
				if (method) return method->Invoke<Material*>(this);
				return nullptr;
			}

			auto GetMaterials() -> std::vector<Material*> {
				static Method* method;
				if (!method) method = Get("UnityEngine.CoreModule.dll")->Get("Renderer")->Get<Method>("get_materials");
				if (method) {
					const auto array = method->Invoke<Array<Material*>*>(this);
					return array->ToVector();
				}
				return {};
			}
		...
		struct Animator : Behaviour {
			...
			// set_speed
			auto SetSpeed(const float value) -> void {
				static Method* method;
				if (!method) method = Get("UnityEngine.AnimationModule.dll")->Get("Animator")->Get<Method>("set_speed");
				if (method) return method->Invoke<void>(this, value);
			}
		};
