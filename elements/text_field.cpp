...
bool input_text_ex(std::string_view label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImU32 bg_color, float rounding, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    struct c_field
    {
        float line_offset;
    ...
    if (!is_multiline)
    {
        draw->rect_filled(window->DrawList, rect.Min, rect.Max, bg_color, SCALE(rounding));
        draw->text_clipped(window->DrawList, var->font.icon[1], rect.Max - ImVec2(size_arg.y, size_arg.y), rect.Max, draw->get_clr(clr->other.accent), label.data(), 0, 0, { 0.5, 0.5 });
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = draw->get_clr(clr->other.accent, render_cursor ? 0.2f : 0.2f);
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw->rect_filled(window->DrawList, rect.Min, rect.Max, bg_color);
                }
        ...
        {
            ImU32 col = draw->get_clr(is_displaying_hint ? clr->text.text_inactive : clr->text.text_active);
            draw->text(window->DrawList, g.Font, g.FontSize, draw_pos - draw_scroll, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            c_state->line_offset = ImLerp(c_state->line_offset, cursor_screen_rect.Min.x - rect.Min.x, gui->fixed_speed(element->text_field.animation_tickrate));
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw->line(window->DrawList, ImVec2(rect.Min.x + c_state->line_offset, cursor_screen_rect.Min.y), ImVec2(rect.Min.x + c_state->line_offset, cursor_screen_rect.GetBL().y), draw->get_clr(clr->other.white));
    ...
    else
    {
        ...
        {
            ImU32 col = draw->get_clr(is_displaying_hint ? clr->text.text_inactive : clr->text.text_active);
            draw->text(window->DrawList, g.Font, g.FontSize, draw_pos, col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
...
bool c_widget::text_field(std::string_view hint, std::string_view label, char* buf, size_t buf_size, const ImVec2& size, ImU32 bg_color, float rounding, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));
    return input_text_ex(label.data(), hint.data(), buf, (int)buf_size, size, bg_color, rounding, flags, callback, user_data);
}

bool input_text_ui(std::string_view label, const char* hint, char* buf, int buf_size, ImU32 bg_color, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    struct c_field
    {
        float line_offset;
    ...
    const ImGuiID id = window->GetID(label.data());
    const ImVec2 label_size = CalcTextSize(label.data(), NULL, true);
    const ImRect rect(window->DC.CursorPos + SCALE(0, 15), window->DC.CursorPos + ImVec2(gui->content_region_avail().x, SCALE(50)));

    ImGuiWindow* draw_window = window;
    ImVec2 inner_size = ImVec2(gui->content_region_avail().x, SCALE(50));
    ImGuiLastItemData item_data_backup;
    if (is_multiline)
    {
        ImVec2 backup_pos = window->DC.CursorPos;
    ...
    {
        ItemSize(ImRect(rect.Min, rect.Max + SCALE(0, 30)), 0);
    ...
    if (!is_multiline)
    {
        draw->rect_filled(window->DrawList, rect.Min, rect.Max, bg_color, SCALE(element->text_field.rounding));
        draw->text_clipped(window->DrawList, var->font.icon[1], rect.Max - SCALE(35, 35), rect.Max, draw->get_clr(clr->other.accent), label.data(), 0, 0, { 0.5, 0.5 });

        draw->line(window->DrawList, rect.Min + SCALE(0, 50), rect.Max + SCALE(0, 15), draw->get_clr(clr->other.element_separator), 1.f);
    }

    const ImVec4 clip_rect(rect.Min.x, rect.Min.y, rect.Min.x + inner_size.x - SCALE(35), rect.Min.y + inner_size.y);
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : rect.Min + ImVec2((SCALE(35) - CalcTextSize(label.data()).y) / 2, (SCALE(35) - CalcTextSize(label.data()).y) / 2);
    ImVec2 text_size(0.0f, 0.0f);
...
bool c_widget::def_text_field(std::string_view hint, std::string_view label, char* buf, size_t buf_size, ImU32 bg_color, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline));
    return input_text_ui(label.data(), hint.data(), buf, (int)buf_size, bg_color, flags, callback, user_data);
}