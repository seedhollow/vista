...
void c_draw::text_clipped(ImDrawList* draw_list, ImFont* font, const ImVec2& pos_min, const ImVec2& pos_max, ImU32 color, const char* text, const char* text_display_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect)
{
    ...
    if (need_clipping)
    {
        ...
        draw->text(draw_list, NULL, 0, pos, color, text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        draw->text(draw_list, NULL, 0, pos, color, text, text_display_end, 0.0f, NULL);
    }
    gui->pop_font();
}
...
void c_draw::rotate_end(float rad, ImDrawList* draw_list, ImVec2 center)
{
...
void c_draw::colored_text(ImFont* font, ImVec2 pos1, ImVec2 pos2, const std::vector<rich_text>& texts, ImVec2 align)
{
    gui->push_font(font);
    ...
    for (const auto& entry : texts)
    {
        ...
        ImVec2 text_size = ImGui::CalcTextSize(entry.text);
        total_text_size.x += text_size.x + ImGui::GetStyle().ItemSpacing.x;
        total_text_size.y = ImMax(total_text_size.y, text_size.y);
    }

    total_text_size.x -= ImGui::GetStyle().ItemSpacing.x;

    // Calculate cursor position based on alignment
    cursor.x = pos1.x + (pos2.x - pos1.x) * align.x - total_text_size.x * align.x;
    cursor.y = pos1.y + (pos2.y - pos1.y) * align.y - total_text_size.y * align.y;
    ...
    for (const auto& entry : texts)
    {
        ...
        ImGui::TextColored(ImColor(entry.color), "%s", entry.text);

        ImVec2 text_size = ImGui::CalcTextSize(entry.text);
        cursor.x += text_size.x + ImGui::GetStyle().ItemSpacing.x;
    }

    gui->pop_font();
}

void c_draw::render_check_mark(ImDrawList* draw_list, ImVec2 pos, ImU32 col, float sz, float stroke_size)
{
    float thickness = stroke_size;
    sz -= thickness * 0.5f;
