...
bool scrollbar_ex(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    // *state = ImLerp(*state, IsItemActive() ? clr->other.accent : hovered ? clr->c_text.text : clr->c_element.layout, gui->fixed_speed(8.f));

    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    else
        grab_rect = ImRect(bb.Min.x + (style.ScrollbarSize / 2.4f), ImLerp(bb.Min.y, bb.Max.y, grab_v_norm), bb.Max.x - (style.ScrollbarSize / 2.4f), ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels);

    draw->rect_filled(window->DrawList, grab_rect.Min - SCALE(4, -GetStyle().WindowPadding.y), grab_rect.Max - SCALE(4, GetStyle().WindowPadding.y), draw->get_clr(clr->other.accent), SCALE(30.f));
...
void render_window_decorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, bool handle_borders_and_resize_grips, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size, ImU32 shadow_col, float shadow_size)
{
    ImGuiContext& g = *GImGui;
    ImGuiStyle& style = g.Style;
    ImGuiWindowFlags flags = window->Flags;
...
bool is_window_open_with_names(ImGuiContext* g)
{
...
bool c_gui::begin(std::string_view name, bool* p_open, ImGuiWindowFlags flags, ImU32 shadow_col, float shadow_size)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // FIXME: User code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please get in contact if you are affected (github #4493)
        {
            ...
            render_window_decorations(window, title_bar_rect, title_bar_is_highlight, handle_borders_and_resize_grips, resize_grip_count, resize_grip_col, resize_grip_draw_size, shadow_col, shadow_size);
