...
struct c_slider {
    ...
    float grab_slide = 0.f, grab_offset = 0.f, circle = 8.f, alpha = 0;
    T slow_value = 0;
};

template<typename T>
bool slider_scalar(std::string_view label, ImGuiDataType data_type, T* p_data, const T* p_min, const T* p_max, const char* format, std::string_view hinting)
{
    ...
    bool hovered = ItemHoverable(slider, id, g.LastItemData.InFlags), held, pressed = ButtonBehavior(ImRect(slider.Min, slider.Max), id, &hovered, &held, NULL);
    ...
    state->grab_slide = ImLerp(state->grab_slide, grab_bb.Min.x - slider.Min.x, gui->fixed_speed(element->slider.animation_tickrate));
    state->alpha = ImLerp(state->alpha, hovered ? 1.f : 0.f, gui->fixed_speed(element->slider.animation_tickrate));
    state->text = ImLerp(state->text, IsItemActive() ? clr->text.text_active : hovered ? clr->text.text_hovered : /*clr->text.text_inactive*/ clr->text.text_active, gui->fixed_speed(8.f));
    state->hint = ImLerp(state->hint, hint_target ? clr->other.accent : clr->text.text_inactive, gui->fixed_speed(8.f));

    {
        draw->rect_filled(window->DrawList, slider.Min, slider.Max, draw->get_clr(clr->slider.layout), SCALE(element->slider.rounding));
        draw->rect_filled_multi_color(window->DrawList, slider.Min, { slider.Min.x + state->grab_slide + SCALE(13), slider.Max.y }, draw->get_clr(clr->other.accent), draw->get_clr(clr->other.accent, 0.5f), draw->get_clr(clr->other.accent, 0.5f), draw->get_clr(clr->other.accent), SCALE(element->slider.rounding));

        draw->rect_filled(window->DrawList, { slider.Min.x + state->grab_slide, slider.GetCenter().y - SCALE(4) }, { slider.Min.x + state->grab_slide + SCALE(14), slider.GetCenter().y + SCALE(4) }, draw->get_clr(clr->other.white), SCALE(element->slider.rounding));
    }
    ...
    if constexpr (std::is_arithmetic_v<T>) {
        ...
        T step = (target_value - state->slow_value) * gui->fixed_speed(element->slider.animation_tickrate);
...

    {
        ...
        draw->text_clipped(window->DrawList, var->font.inter_medium[0], rect.Min + ImVec2(0, padding), rect.Max, draw->get_clr(clr->text.text_active), value_buf, NULL, NULL, { 1.0, 0.0 });

        if (hinting[0])
        {
            draw->text_clipped(window->DrawList, var->font.inter_medium[0], rect.Min + ImVec2(CalcTextSize(label.data()).x + SCALE(10), padding), rect.Max, draw->get_clr(state->hint), "[ ? ]", NULL, NULL, { 0.0, 0.0 }, NULL);
...
bool c_widget::slider_float(std::string_view label, float* v, float v_min, float v_max, const char* format, std::string_view hinting)
{
    return slider_scalar<float>(label, ImGuiDataType_Float, v, &v_min, &v_max, format, hinting);
}

bool c_widget::slider_int(std::string_view label, int* v, int v_min, int v_max, const char* format, std::string_view hinting)
{
    return slider_scalar<int>(label, ImGuiDataType_S32, v, &v_min, &v_max, format, hinting);
}
